...
describe("Newman and htmlextra run from a script", function () {
  var outFile = "out/newman-report.htmlextra.html",
    invalidCollectionNameFolder =
      "test/requests/collection_with_name_containing_invalid_characters",
  ...
  files.forEach(function (file) {
    let collectionFile = JSON.parse(
        fs.readFileSync(invalidCollectionNameFolder + "/" + file, "utf8")
      ),
      ...
      checkFileExistance = function (input, outputFile) {
        let status,
          output,
          timeStamp = outputFile[0].match(
            /-\d{4}-\d{2}-\d{2}-\d{2}-\d{2}-\d{2}-\d{3}-\d.html$/g
          );
        ...
        if (outputFile.length !== 0) {
          ...
          status =
            output === null ? `Expected ${input}${timeStamp[0]}` : output[0];
    ...
    it(
      "should correctly generate the html report for collection" +
        "name (" +
        collectionFile.info.name +
        ")",
      function (done) {
        newman.run(
          {
  ...
  it("should correctly generate html report for a formdata POST request", function (done) {
    newman.run(
      ...
      function (err, summary) {
        ...
        expect(summary.collection.name).to.equal(
          "simple-post-request-with-formdata"
        );
        expect(summary.run.stats.iterations.total).to.equal(1);
        expect(
          summary.run.executions[0].item.request.body.formdata.members[0].key
        ).to.equal("myKey");
        expect(
          summary.run.executions[0].item.request.body.formdata.members[0].value
        ).to.equal("myValue");
        expect(
          summary.run.executions[0].item.request.body.formdata.members[0].src
        ).to.be.undefined;
        expect(
          summary.run.executions[0].item.request.body.formdata.members[1].key
        ).to.equal("anotherKey");
        expect(
          summary.run.executions[0].item.request.body.formdata.members[1].value
        ).to.equal("anotherValue");
        expect(
          summary.run.executions[0].item.request.body.formdata.members[1].src
        ).to.be.undefined;
        expect(
          summary.run.executions[0].item.request.body.formdata.members[2].key
        ).to.equal("demoFile");
        expect(
          summary.run.executions[0].item.request.body.formdata.members[2].src
        ).to.equal("demoFile.jpg");
        expect(
          summary.run.executions[0].item.request.body.formdata.members
        ).to.have.lengthOf(3);
        fs.stat(outFile, done);
      }
    );
  ...
  it("should correctly generate html report for a urlencoded POST request", function (done) {
    newman.run(
      ...
      function (err, summary) {
        ...
        expect(summary.collection.name).to.equal(
          "simple-post-request-with-urlencoded"
        );
        expect(summary.run.stats.iterations.total).to.equal(1);
        expect(
          summary.run.executions[0].item.request.body.urlencoded.members[0].key
        ).to.equal("myKey");
        expect(
          summary.run.executions[0].item.request.body.urlencoded.members[0]
            .value
        ).to.equal("myValue");
        expect(
          summary.run.executions[0].item.request.body.urlencoded.members
        ).to.have.lengthOf(1);
        fs.stat(outFile, done);
      }
    );
    ...
    newman.run(
      {
        ...
        reporters: ["htmlextra-extended"],
        reporter: {
          "htmlextra-extended": { export: outFile, testPaging: true },
        },
        // eslint-disable-next-line consistent-return
      },
      function (err, summary) {
  ...
  it("should correctly generate the html report for a runs using an environment file", function (done) {
    newman.run(
      ...
      function (err, summary) {
        ...
        expect(
          summary.run.stats.assertions.total,
          "should have 1 assertion"
        ).to.equal(1);
        expect(
          summary.run.stats.assertions.failed,
          "should have 0 failed assertions"
        ).to.equal(0);
        fs.stat(outFile, done);
      }
    );
    ...
    newman.run(
      {
        ...
        reporters: ["htmlextra-extended"],
        reporter: {
          "htmlextra-extended": { export: outFile, showEnvironmentData: true },
        },
        // eslint-disable-next-line consistent-return
      },
      function (err, summary) {
    ...
    newman.run(
      {
        ...
        reporter: {
          "htmlextra-extended": {
            ...
            skipEnvironmentVars: ["secretVariable"],
          },
        },
        // eslint-disable-next-line consistent-return
      },
  ...
  it("should correctly generate the html report for a failed run", function (done) {
    newman.run(
      ...
      function (err, summary) {
        ...
        expect(
          summary.run.stats.assertions.failed,
          "should have 2 failed assertions"
        ).to.equal(2);
        expect(summary.run.failures, "should have 2 failures").to.have.lengthOf(
          2
        );
        fs.stat(outFile, done);
      }
    ...
    newman.run(
      {
        ...
        reporters: ["htmlextra-extended"],
        reporter: {
          "htmlextra-extended": { export: outFile, showOnlyFails: true },
        },
        // eslint-disable-next-line consistent-return
      },
      function (err, summary) {
  ...
  it("should correctly generate the html report for a run with console logs", function (done) {
    newman.run(
      ...
      function (err, summary) {
        ...
        expect(err).to.be.null;
        expect(
          summary.consoleLogs[key],
          "should have 2 console log messages"
        ).to.have.lengthOf(2);
        fs.stat(outFile, done);
      }
    );
  ...
  it("should correctly produce the html report for a run with all test types", function (done) {
    newman.run(
      ...
      function (err, summary) {
        ...
        expect(summary.consoleLogs, "should have no console log messages").to.be
          .undefined;
        // individual request data
        expect(summary.run.executions[0].assertions[0].assertion).to.equal(
        ...
        expect(summary.run.executions[0].assertions[1].assertion).to.equal(
          "This is a failing test"
        );
        expect(summary.run.executions[0].assertions[1].skipped).to.be.false;
        // eslint-disable-next-line max-len
        expect(summary.run.executions[0].assertions[1].error.message).to.equal(
          "expected response to have status code 201 but got 200"
        );
        expect(summary.run.executions[0].assertions[2].assertion).to.equal(
    ...
    newman.run(
      {
        ...
        reporters: ["htmlextra-extended"],
        reporter: {
          "htmlextra-extended": { skipHeaders: "testHeader", export: outFile },
        },
        // eslint-disable-next-line consistent-return
      },
      function (err, summary) {
    ...
    newman.run(
      {
        ...
        reporter: {
          "htmlextra-extended": {
            skipHeaders: ["testHeader", "host", "User-Agent"],
            export: outFile,
          },
        },
        // eslint-disable-next-line consistent-return
    ...
    newman.run(
      {
        ...
        reporters: ["htmlextra-extended"],
        reporter: {
          "htmlextra-extended": { omitHeaders: true, export: outFile },
        },
        // eslint-disable-next-line consistent-return
      },
      function (err, summary) {
    ...
    newman.run(
      {
        ...
        reporter: {
          "htmlextra-extended": {
            omitHeaders: true,
            export: outFile,
            skipRequests: "request1,request4",
          },
        },
        // eslint-disable-next-line consistent-return
      },
