...
PostmanHTMLExtraReporter = function (newman, options, collectionRunOptions) {
    ...
    handlebars.registerHelper('getScenarioNumber', function (request, rootNode) {
        if (options.scenarioNumbering === true) {
            var prevRequest = rootNode.data.root.prevRequest;
            var scenarioCount = rootNode.data.root.scenarioCount;
            if (prevRequest === undefined || prevRequest !== request) {
                rootNode.data.root.prevRequest = request;
                rootNode.data.root.scenarioCount = 1;
                return ' Scenario ' + 1;
            }
            rootNode.data.root.prevRequest = request;
            scenarioCount++;
            rootNode.data.root.scenarioCount = scenarioCount;
            return ' Scenario ' + scenarioCount;
        }
        return '';
    });

    handlebars.registerHelper('getIterationName', function (value) {
        if (options.iterationNames === undefined || options.iterationNames.length === 0 || options.iterationNames.length <= value) {
            return value + 1;
        }
        else if (Array.isArray(options.iterationNames)) {
            if (options.iterationNames[value].length > value) {
                return options.iterationNames[value].toUpperCase();
            }
            return value + 1;
        }
        else if ((typeof options.iterationNames) === 'string') {
            if (options.iterationNames.replaceAll('"', '').split(',').length > value) {
                return (options.iterationNames.replaceAll('"', '').split(',')[value]).toUpperCase();
            }
            return value + 1;
        }
        return options.iterationNames[value];
    });

    handlebars.registerHelper('setScenarioNumber', function (request, rootNode) {
        if (options.scenarioNumbering === true) {
            var prevRequest = rootNode.data.root.prevRequest;
            var scenarioCount = rootNode.data.root.scenarioCount;
            if (prevRequest === undefined || prevRequest !== request) {
                rootNode.data.root.prevRequest = request;
                rootNode.data.root.scenarioCount = 1;
                rootNode.data.root.failureScenarioNumber = ' Scenario ' + 1;
            }
            else {
                rootNode.data.root.prevRequest = request;
                scenarioCount++;
                rootNode.data.root.scenarioCount = scenarioCount;
                rootNode.data.root.failureScenarioNumber = ' Scenario ' + scenarioCount;
            }
        }
        return '';
    });

    handlebars.registerHelper('AppendTestNames', function (assertions) {
        if (options.AppendTestNames === true) {
            var name = ' - ';
            for (let i = 0; i < assertions.length; i++) {
                name += assertions[i].name;
    ...
    handlebars.registerHelper('getFailureScenarioNumber', function (rootNode) {
        return rootNode.data.root.failureScenarioNumber;
    });

    handlebars.registerHelper('getIterationNameforTabs', function () {
        return options.iterationNames;
    });
    ...
    newman.on('beforeDone', function () {
        ...
        this.exports.push({
            ...
            content: compiler({
                ...
                timestamp: moment().tz(timezone).format('dddd, DD MMMM YYYY HH:mm:ss z'),
                ...
                iterationNames: options.iterationNames || [],
                scenarioNumbering: options.scenarioNumbering || false,
                AppendTestNames: options.AppendTestNames || false,
                summary: {
